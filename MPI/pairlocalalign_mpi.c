...
#if 0 // Create problematic cache FOR TEST.  DISABLE here for usual use.
//	system( "rm -rf hat3dir/" );
	system( "while true\n do\n rm -rf hat3dir/ && break\n echo retrying 1>&2\n sleep 2\n done\n" );
//	reporterr( "before bcast\n" );
//	system( "hostname 1>&2" );
	MPI_Barrier(MPI_COMM_WORLD);
//	reporterr( "after bcast\n" );
//	system( "hostname 1>&2" );
#endif
...
//		system( "mkdir -p hat3dir/" ); // toriaezu
		for( i=0; i<njob-1; i+=HAT3NODEBLOCK ) 
		{
			sprintf( fn, "mkdir -p hat3dir/%d-", i );
			system( fn );
		}
		free( fn );
//		reporterr( "Created hat3dir by rank0\n" );
//		system( "ls -ltr hat3dir/* 1>&2" );
	}
#if 0   // -> mltalln9_mpi.c
	// synchronize all processes to make sure hat3dir/ is cached in all nodes
	MPI_Barrier(MPI_COMM_WORLD);
//	system( "echo 3 > /proc/sys/vm/drop_caches 1>&2" ); // only for root
//	system( "ls -ltrR hat3dir/ 1>&2" );
//	system( "hostname 1>&2" );
	fn = calloc( 100, sizeof( char ) );
	for( i=0; i<njob-1; i+=HAT3NODEBLOCK ) 
	{
		sprintf( fn, "mkdir -p hat3dir/%d-", i );
//		sprintf( fn, "touch hat3dir/%d-/", i );
		system( fn );
	}
	free( fn );
#endif
...
//			reporterr( "seq = %s\n", seq[i] );
		...
		// synchronize uselh because uselh is assigned using rand()
		MPI_Bcast(uselh, njob, MPI_INT, 0, MPI_COMM_WORLD);
...
//
	...
	if(my_rank==0){
		fp = fopen( "hat3dir/tree", "w" );
		treeout_bin( fp, njob, topol, len, dep, nfilesfornode );
		fclose( fp );

		fp = fopen( "hat3dir/uselh", "w" );
